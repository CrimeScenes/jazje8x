...
local function isWithinTargetZone(position, size)
    local screenPos, onScreen = Camera:WorldToViewportPoint(position)
    local mouseX, mouseY = getMousePosition()

    -- Calculate if the mouse is within the target zone based on the bounding box size
    local left = screenPos.X - size.x / 2
    local right = screenPos.X + size.x / 2
    local top = screenPos.Y - size.y / 2
    local bottom = screenPos.Y + size.y / 2

    return onScreen and mouseX >= left and mouseX <= right and mouseY >= top and mouseY <= bottom
end
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    -- Get the target size from the settings (x, y, z values)
                    local size = shared.Forbidden.TriggerBot.Settings.DetectionZone
                    
                    -- Check if the mouse is within the target area for the current body part
                    if isWithinTargetZone(part.Position, size) then
